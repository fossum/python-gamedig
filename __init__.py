...
class GameDig:
    def __init__(self, game_id: str, host: str, port: int = None, given_port: int = None) -> None:
        """"""
    ...
    async def query(self, timeout: float = 5.0, max_attempts: int = 3, encoding: str = "utf8"):
        """
        ...
        protocol_module = importlib.import_module(f"gamedig.protocols.{self._game.protocol}")
        protocol_class = getattr(protocol_module, self._game.protocol.capitalize())
        self._protocol = protocol_class(self, timeout=timeout, max_attempts=max_attempts, encoding=encoding)
    ...
    def _load_game_definition(self) -> None:
        ...
        for game_name, data in games.items():
            game_definitions[game_name.lower()] = (
                GameDefinition(
        ...
        if self.port is None:
            raise GameDigException(f"Game '{self.game_id}' port not defined in games.json or given_port parameter.")
